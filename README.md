# Portfolio
Hi, my name is Isabella Mo!

I'm studying Electrical Engineering at UC Davis with a concentration in Mixed-Signal Systems. 

This summer, I'm continuing my research in low-power digital design as a National Science Foundation ExLENT Fellow in the UC Davis BE-STARSE Program. Additionally, I'm working as a Student Assistant at the UC Davis Center for Nano-Micro Manufacturing (CNM2) to expand my knowledge of hardware design through maintaining cleanroom facilities and advanced microfabrication processing machinery.

## Research Projects
### [Hardware-Efficient Digital Filter Design](digital-filter.md)
**BE-STARSE Program** | March 2026 - Present | Enhance Micro Electro-Mechanical Systems (MEMS) microphone signals from aeroacoustic wind tunnel testing.

### [Lifting Surfaces Sensors](lifting-sensors.md)
**Micropower Circuits & Systems Group** | November 2025 - June 2026 | Explored characterizating the behavior of MEMS microphone and graphite sensors to inform recommendations for optimizing airflow efficiency.

## Extracurricular Projects
### [Telementry/Health PCB](telemetry-health.md)
**Cyclone RoboSub at UC Davis** | November 2025 - Present | Design custom system detecting leaks within robot, temperature of electronics, and underwater acoustic signals. 

## Course Projects

### [Autonomous Acoustic Navigation System](acoustic-navigation.md)
**EEC 010: Introduction to Digital & Analog Systems** | April 2026 - June 2026 | Assembled autonomous robot utilizing audio electronic system to localize and navigate toward acoustic source.

### [Matrix Multiplication Accelerator](matrix-multiplier.md)
**EEC 180: Digital Systems II** | February 2026 – March 2026 | Optimized Verilog Multiply-Accumulate units and Dual-Port RAM for 2x data throughput.

### [FPGA Dice Game](dice-game.md)
**EEC 018: Digital Systems I** | November 2025 – December 2025 | Mealy Finite State Machine design with One-Hot encoding and Binary Coded Decimal arithmetic.

### [Jelly Bean Counting Scale](precision-scale.md)
**EEC 001: Introduction to Electrical & Computer Engineering** | May 2025 – June 2025 | 0.5g resolution measurement system with INA125 amplification and electromagnetic interference mitigation.

### ["Cowtastrophe" Multi-User Game](cowtastrophe.md)
**ENG 006: Engineering Problem-Solving** | February 2025 – March 2025 | MATLAB OOP-based engine synchronized via ThingSpeak Cloud API.

### [Raynaud’s Syndrome Preventative Armband](raynauds-armband.md)
**ENG 003: Introduction to Engineering Design** | October 2024 – December 2024 | Arduino-based wearable prototype for medically-assistive thermal alerts.

## Professional Experience

### Student Assistant
#### Center for Nano-Micro Manufacturing (CNM2)
**June 2026 - Present**

### National Science Foundation ExLENT Fellow
#### BE-STARSE Program
**March 2026 - Present** | Conduct mixed-signal systems research aligned with national CHIPS and Science Act initiatives under the mentorship of Dr. Rajeevan Amirtharajah.

### Undergraduate Research Assistant
#### Micropower Circuits & Systems Group
**November 2025 - Present** | Execute wind tunnel testing on 3D-printed airfoils using MEMS microphones, analyzing acoustic signals to map airflow transitions and designing sensor array architectures to optimize aerodynamic efficiency.

## Extracurriculars

### Incoming Electrical Lead
#### Cyclone RoboSub at UC Davis
**May 2026 - Present** | Lead electrical subteam task management for PCB design, manufacturing, verification, and system integration for internationally competitive autonomous underwater vehicles. Upgrade onboarding resources to include electrical project summaries, software setup guides, and introductory PCB design tutorials.

### Startup Coordinator
#### IEEE Student Branch at UC Davis
**March 2025 - Present** | Host quarterly computer vision and vibe coding workshops with the UC Davis Student Startup Center for groups of 20-50 undergraduate and graduate students. Foster technical and business growth within the UC Davis College of Engineering by connecting participants to professors teaching student-led engineering entrepreneurship courses.

### Electrical Subteam Member
#### Formula Racing at UC Davis
**October 2024 - August 2025** | Assembled and validated automotive electrical systems, utilizing reflow soldering for Battery Management System Node Boards. Tested wiring harness durability in high-vibration environments.

## Let's Connect

Please contact me on [LinkedIn](https://linkedin.com/in/isabella-mo/) to discuss any engineering internships, opportunities, or projects. I'm also happy to answer any questions!
